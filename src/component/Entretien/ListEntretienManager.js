...
class ListEntretien extends Component {
  ...
  retrieveEntretien() {
    EntretienService.count(undefined, this.props.idUser).then((resp) => {
      let nbPage = Math.ceil(resp.data / this.state.itemsPerPage)
      this.setState({ pageCount: nbPage })
    }).catch((e) => { console.log(e) });
    EntretienService.getAllEntretiensByPage(this.state.currentPage, this.state.itemsPerPage, undefined, this.props.idUser)
      .then(response => {
        this.setState({
